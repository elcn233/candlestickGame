...
##### 该消息需带上通过、拒绝两个按钮。
##### 当管理员点击通过按钮：
增加用户余额，并且机器人在群内回复用户这条“上分申请”消息，提示”上分成功“的文案
##### 当管理员点击拒绝按钮：
对用户余额不进行任何操作，并且机器人在群内回复用户这条“上分申请”消息，提示”上分失败“的文案

#### API自动上分：
API文档请参考[13.用户自动上分API文档](#13用户自动上分API文档)
##### 当API返回上分成功：
...
##### 该消息需带上通过、拒绝两个按钮。
##### 当管理员点击通过按钮：
扣除用户余额，并且机器人在群内回复用户这条“下分申请”消息，提示”下分成功“的文案
##### 当管理员点击拒绝按钮：
对用户余额不进行任何操作，并且机器人在群内回复用户这条“下分申请”消息，提示”下分失败“的文案

#### API自动下分：
API文档请参考[14.用户自动下分API文档](#14用户自动下分API文档)
##### 当API返回下分成功：
...
先判断发送命令的用户是否具有管理员权限
判断上分命令格式是否正确
若上分命令格式正确且具有管理员权限，则根据“上分成功”回复消息，并为用户添加余额
若不具有管理员权限，则根据“无管理员权限文案”回复消息，不操作用户余额
若上分命令格式错误，则根据“管理员上分文案”回复消息，不操作用户余额

## 11.管理员下分开发文档
先判断发送命令的用户是否具有管理员权限
判断下分命令格式是否正确
若下分命令格式正确且具有管理员权限，则根据“下分成功”回复消息，并为用户扣除余额
若不具有管理员权限，则根据“无管理员权限文案”回复消息，不操作用户余额
若下分命令格式错误，则根据“管理员下分文案”回复消息，不操作用户余额
